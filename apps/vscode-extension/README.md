...
- 「历史回放」按当前分支的提交顺序重放项目结构的成长：文件先归属地图节点，未覆盖的按目录
  推断成分组，节点随代码量变大、本次提交改动的节点高亮，位置全程固定，可播放、暂停、逐帧、
  变速和拖动进度；只读 Git（`log` / `ls-tree`），被截断的提交数和按目录推断的节点数会如实标注，
  回放期间到达的 AI 补丁不改画面，退出后照常呈现。
