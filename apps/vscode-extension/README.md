...
> `God View: Copy Agent Setup` 保留为手动接入兜底。
>
...
- Agent 可提交三类声明式讲解，用户可播放、暂停、逐步导航、变速、重播和退出；
  减少动态效果设置会关闭镜头补间。
- 用户可在节点上创建解释/风险/修改标注，Agent 回答以纯文本和结构化证据原位显示；
...
- 中断只结束 God View 的 ChangeSet 并保留现有文件修改；MCP guided-call 模式无法终止外部 Agent 进程。
...
- 不主动调用 Agent。Codex/Claude Code 目前均为 MCP 引导调用，不读取密钥或登录态；
- monitored 模式不能强制沙箱化外部 Agent。它能发现越界 Diff，但不能阻止写入发生；
...
会话启动时加载，旧会话不会热加载 `get_map` 等工具。重开后先要求 Agent 调用 `get_map`，
确认 workspace、branch 与 revision，再生成并粘贴初始化任务。自动配置失败时再使用
`God View: Copy Agent Setup` 获取手动命令。
...
推荐在 Extension Development Host 中执行 **God View: Configure Agent MCP**，确认后退出并
重开 Agent。下面的命令只用于调试手动接入：
